...
def check_industry_job_completions(jobs: List[Dict[str, Any]], char_name: str) -> None:
    ...
    upcoming_completions = [
        ...
        for job in jobs
        if "end_date" in job
        and now <= datetime.fromisoformat(job["end_date"].replace("Z", "+00:00")) <= now + timedelta(hours=24)
    ]
...
def check_planet_extraction_completions(planet_details: Dict[str, Any], char_name: str) -> None:
    ...
    upcoming_extractions = [
        ...
        and now <= datetime.fromisoformat(pin["expiry_time"].replace("Z", "+00:00")) <= now + timedelta(hours=24)
    ]
...
async def process_character_data(
    char_id, token_data, wp_post_id_cache, blueprint_cache, location_cache, structure_cache, failed_structures, args
):
    """
...
async def process_character_blueprints_from_assets(
    char_id: int,
    access_token: str,
    char_name: str,
    wp_post_id_cache: Dict[str, Any],
...
async def process_character_blueprints_from_industry_jobs(
    char_id: int,
    access_token: str,
    char_name: str,
    wp_post_id_cache: Dict[str, Any],
