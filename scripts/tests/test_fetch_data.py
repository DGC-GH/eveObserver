...
from api_client import ApiConfig, ESIApiError, ESIAuthError, ESIRequestError, fetch_esi, fetch_public_esi
from character_processor import check_industry_job_completions, check_planet_extraction_completions
from fetch_data import collect_corporation_members
...
class TestApiConfig:
    ...
    @patch.dict(
        os.environ, {"ESI_TIMEOUT": "invalid", "ESI_MAX_RETRIES": "not_a_number", "ESI_MAX_WORKERS": "also_invalid"}
    )
...
class TestCollectCorporationMembers:
    ...
    @pytest.mark.asyncio
    async def test_collect_corporation_members_valid_tokens(
        self, mock_update_wp, mock_fetch_char, mock_save, mock_refresh, mock_load_tokens
    ):
    ...
    @patch("fetch_data.update_character_in_wp")
    @pytest.mark.asyncio
    async def test_collect_corporation_members_expired_token_refresh(
        self, mock_update_wp, mock_fetch_char, mock_save, mock_refresh, mock_load_tokens
    ):
...
class TestIndustryJobCompletions:
    ...
    def test_check_industry_job_completions_upcoming(self, mock_datetime, mock_send_email):
        ...
        jobs = [
            {"job_id": 1, "end_date": (now + timedelta(hours=20)).isoformat().replace("+00:00", "Z"), "activity_id": 1},
            {
                "job_id": 2,
...
class TestBlueprintProcessing:
    ...
    async def test_update_blueprint_in_wp_new_blueprint(
        self,
        mock_set_cache,
        mock_fetch_icon,
        mock_fetch_esi,
        ...
        mock_wp_request.side_effect = [
            ...
            {
                "id": 123,
                "title": {"rendered": "Test Blueprint BPO 10/20 (Test Station) – ID: 12345"},
                "meta": {},
            },  # Created post
        ]
    ...
    async def test_update_blueprint_in_wp_structure_location(
        self,
        mock_fetch_esi_auth,
        mock_fetch_esi_pub,
        mock_wp_request,
    ...
    @patch("data_processors.load_failed_structures")
    @patch("data_processors.load_wp_post_id_cache")
    async def test_process_blueprints_parallel_success(
        self, mock_load_wp_cache, mock_load_failed, mock_load_struct, mock_load_loc, mock_load_bp, mock_update_bp
    ):
        ...
        blueprints = [{"item_id": 1, "type_id": 1001}, {"item_id": 2, "type_id": 1002}, {"item_id": 3, "type_id": 1003}]
    ...
    @patch("data_processors.load_failed_structures")
    @patch("data_processors.load_wp_post_id_cache")
    async def test_process_blueprints_parallel_with_exceptions(
        self, mock_load_wp_cache, mock_load_failed, mock_load_struct, mock_load_loc, mock_load_bp, mock_update_bp
    ):
...
class TestWordPressAPIIntegration:
    ...
    async def test_wp_request_authentication_error(self, mock_rate_limiter, mock_circuit_breaker, mock_get_session):
        ...
        from api_client import WordPressAuthError, wp_request
