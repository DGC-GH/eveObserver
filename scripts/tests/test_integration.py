...
from blueprint_processor import extract_blueprints_from_assets
from contract_processor import process_character_contracts, update_contract_in_wp
from corporation_processor import fetch_corporation_data, update_corporation_in_wp
...
class TestBlueprintProcessorIntegration:
    ...
    async def test_update_blueprint_in_wp_full_integration(
        self,
        mock_load_bp,
        mock_load_loc,
        mock_load_struct,
    ...
    @patch("cache_manager.load_structure_cache")
    @patch("cache_manager.load_failed_structures")
    async def test_process_blueprints_parallel_integration(
        self, mock_load_failed, mock_load_struct, mock_load_loc, mock_load_bp, mock_process_parallel, mock_fetch_esi
    ):
...
class TestCharacterProcessorIntegration:
    ...
    async def test_fetch_character_data_integration(
        self, mock_save_cache, mock_load_cache, mock_wp_request, mock_fetch_esi
    ):
        """Test character data fetching integration."""
        # Setup mocks
        mock_load_cache.return_value = {}
        mock_fetch_esi.return_value = {"name": "Test Character", "corporation_id": 1001, "alliance_id": 2001}
        mock_wp_request.return_value = {"id": 123, "title": {"rendered": "Test Character"}}

        result = await fetch_character_data(123, "test_token")

        expected = {"name": "Test Character", "corporation_id": 1001, "alliance_id": 2001}
    ...
    @patch("cache_manager.load_cache")
    @patch("cache_manager.save_cache")
    async def test_update_character_in_wp_integration(
        self, mock_save_cache, mock_load_cache, mock_wp_request, mock_fetch_public
    ):
        ...
        char_data = {"name": "Test Character", "corporation_id": 1001}
...
class TestContractProcessorIntegration:
    ...
    async def test_update_contract_in_wp_integration(
        ...
        contract_data = {"contract_id": 12345, "issuer_id": 123, "assignee_id": 456}
...
class TestCorporationProcessorIntegration:
    ...
    @patch("cache_manager.load_cache")
    @patch("cache_manager.save_cache")
    async def test_fetch_corporation_data_integration(
        self, mock_save_cache, mock_load_cache, mock_wp_request, mock_fetch_esi
    ):
        """Test corporation data fetching integration."""
        # Setup mocks
        mock_load_cache.return_value = {}
        mock_fetch_esi.return_value = {"name": "Test Corporation", "ticker": "TEST", "member_count": 100}

        result = await fetch_corporation_data(1001, "test_token")

        expected = {"name": "Test Corporation", "ticker": "TEST", "member_count": 100}
        assert result == expected
...
class TestErrorHandlingIntegration:
    ...
    @patch("data_processors.wp_request")
    @patch("data_processors.fetch_public_esi")
    async def test_blueprint_processing_error_handling(
        self,
        mock_fetch_esi,
        mock_wp_request,
        mock_set_cache,
