...
from fetch_data import get_wp_auth, generate_contract_title, load_blueprint_cache
...
def update_contract_titles():
    ...
    response = requests.get(f"{WP_BASE_URL}/wp-json/wp/v2/eve_contract",
                          auth=get_wp_auth(),
    ...
    for contract in contracts:
        ...
        if current_title != new_title:
            ...
            update_response = requests.post(f"{WP_BASE_URL}/wp-json/wp/v2/eve_contract/{post_id}",
                                          json=update_data,
                                          auth=get_wp_auth())
            ...
            if update_response.status_code in [200, 201]:
                ...
                print(f"  ✗ Failed to update contract {contract_id}: {update_response.status_code} - {update_response.text}")
        else:
