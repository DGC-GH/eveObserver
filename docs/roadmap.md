# EVE Observer - Project Overview

EVE Observer is a comprehensive web-based dashboard for EVE Online industrialists, built on WordPress with Advanced Custom Fields (ACF) plugin. It integrates with the EVE ESI API to provide real-time monitoring and analysis of industrial activities across multiple characters and corporations.

## Core Features

### 🔧 **Blueprint Management**
- Tracks BPOs and BPCs from multiple sources (character blueprints, assets, industry jobs, contracts)
- Displays material efficiency (ME) and time efficiency (TE) levels
- Shows location information and ownership details
- Supports pagination for large blueprint collections

### 🏭 **Industry Job Monitoring**
- Real-time tracking of active industry jobs
- Automatic email alerts for job completions (24-hour window)
- Supports both character and corporation jobs
- Displays job progress and completion times

### 🪐 **Planetary Interaction (PI)**
- Colony management across multiple characters
- Extraction timer monitoring with alerts
- Planet details including pin configurations
- Resource tracking and optimization insights

### 📊 **Dashboard & Analytics**
- Interactive charts showing character, blueprint, and planet counts
- Paginated data tables for large datasets
- Real-time timers for industry jobs and PI extractions
- Responsive design for desktop and mobile use

### 🔐 **Multi-Character Support**
- OAuth-based authentication for multiple EVE characters
- Corporation-level data access where permitted
- Automatic token refresh and management
- Configurable corporation filtering

## Technical Architecture

### Backend
- **Python Scripts**: ESI API integration, data fetching, and WordPress synchronization
- **WordPress REST API**: Custom post types for data storage
- **Caching System**: Intelligent caching for API responses and WordPress post IDs
- **Automated Updates**: Cron-based daily data refreshes

### Frontend
- **WordPress Dashboard**: ACF-powered custom post types
- **JavaScript/AJAX**: Dynamic pagination and real-time updates
- **Chart.js**: Data visualization and analytics
- **Responsive Design**: Mobile-friendly interface

### Security & Compliance
- Read-only ESI API access (no game automation)
- Secure token storage and management
- Environment-based configuration
- CCP policy compliant (manual in-game actions only)

## Configuration

The system uses centralized configuration with environment variables:
...
1. **Setup Environment**: Install dependencies and configure `.env`
2. **Register ESI App**: Create application at developers.eveonline.com
3. **Authorize Characters**: Run `python esi_oauth.py authorize`
4. **Fetch Data**: Execute `python fetch_data.py`
5. **View Dashboard**: Access WordPress dashboard with real-time data

## Development

- **Testing**: Comprehensive pytest suite with 9 test cases
- **Code Quality**: Modular design with focused helper functions
- **Documentation**: Detailed setup and troubleshooting guides
- **Version Control**: Git-based development with feature branches

## Compliance & Ethics

EVE Observer is designed for out-of-game analysis and optimization only:
- ✅ Read-only ESI API usage
- ✅ Manual in-game actions required
- ✅ No automation of game mechanics
- ✅ CCP policy compliant

This tool enhances the EVE Online experience by reducing industrial management tedium while maintaining fair play standards.
